...
The Streams API provides a web-standard interface for reading, writing, and
transforming streaming data. It enables efficient processing of large datasets
and real-time data without loading everything into memory.
...
- `transform` (object): Transform stream with `readable` and `writable`
  properties
