...
import { Menu, X, Search, Book, ArrowLeft, ExternalLink } from "lucide-preact";
    ...
    <div>
      ...
      >
        ...
        <div class="px-6 py-4 border-b border-surface0/50">
            ...
            <div class="flex items-center space-x-3">
              ...
              <div>
                <h1 class="text-xl font-bold text-text">
                  <a href="/" class="hover:text-subtext0 transition-colors">Andromeda</a>
                </h1>
          ...
          {/* Search */}
          <div class="relative">
            <Search class="absolute left-3 top-1/2 transform -translate-y-1/2 text-subtext0" size={18} />
            <input
              type="text"
          ...
          {searchResults ? (
            ...
            searchResults.length > 0 ? (
                ...
                {searchResults.map((result: any) => (
                  ...
                  >
                    ...
                    <div class="flex-1 min-w-0">
                      ...
                      <div class="text-xs text-subtext0 truncate">{result.section}</div>
                    </div>
                  </a>
                ))}
              </div>
              ...
              <div class="text-center py-12">
                ...
                <div class="text-subtext1 text-xs mt-1">Try a different search term</div>
              </div>
            )
          ) : (
            /* Regular navigation */
            <div class="space-y-2">
              {filteredData.map(({ name: topicName, children: topicChildren }: any) => {
                const isExpanded = expandedSections.has(topicName);
                const hasCurrentPage = topicChildren.some((child: any) => child.path === path);
                ...
                return (
                  <div key={topicName} class="space-y-1">
                    ...
                    >
                      <div class="flex items-center space-x-3">
                        <span class="text-lg">{iconMap[topicName] || "📄"}</span>
                        <span class="uppercase tracking-wider">{topicName}</span>
                      </div>
                      <span class={`transform transition-transform duration-200 ${isExpanded ? "rotate-90" : ""}`}>
                        ›
                      </span>
                    </button>
                    ...
                    {isExpanded && (
                      <div class="ml-4 space-y-1 border-l-2 border-surface1/50 pl-4">
                        {topicChildren.map(({ name: routeName, path: routePath, id: routeId }: any) => (
                          <a
                            key={routeId}
                            href={routePath}
                            onClick={closeNav}
