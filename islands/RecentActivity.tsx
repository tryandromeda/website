...
import { GitCommit, Tag, MessageCircle } from "lucide-preact";
...
export default function RecentActivity() {
  ...
  useEffect(() => {
    const fetchRecentActivity = async () => {
      try {
        const commitsResponse = await fetch('https://api.github.com/repos/tryandromeda/andromeda/commits?per_page=3');
        if (commitsResponse.ok) {
          const commitsData = await commitsResponse.json();
          setCommits(commitsData);
        }

        const releasesResponse = await fetch('https://api.github.com/repos/tryandromeda/andromeda/releases?per_page=2');
          ...
          <div class="space-y-3">
            {[...Array(4)].map((_, i) => (
              <div key={i} class="flex items-center gap-3">
                <div class="w-8 h-8 bg-surface1 rounded-full flex-shrink-0"></div>
                <div class="flex-1">
